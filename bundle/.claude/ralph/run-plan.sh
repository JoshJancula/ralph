...
# Requires Claude Code CLI (`claude`); see https://code.claude.com/docs/en/headless
# Switch plans by editing plan-runner.json or pass --plan (same flags as Cursor ralph).
#
# Usage:
#   .claude/ralph/run-plan.sh                    # run from repo root (plan from config)
#   .claude/ralph/run-plan.sh /path/repo         # run from specific repo
#   .claude/ralph/run-plan.sh --plan OTHER.md    # use OTHER.md as plan (repo root)
#   .claude/ralph/run-plan.sh --plan docs/plan.md /path/repo
#   .claude/ralph/run-plan.sh --agent research   # prebuilt agent from .claude/agents/<name>/
#   .claude/ralph/run-plan.sh --select-agent     # interactive pick (TTY)
#   .claude/ralph/run-plan.sh --non-interactive --agent research --plan PLAN.md
...
#
# Config (first match): .claude/ralph/plan-runner.json, else shared .ralph/plan-runner.json
#   { "plan": "PLAN.md", "model": "sonnet" }   plan path and optional default model
#
# Native agent teams (multi-session Claude Code): enable with
#   CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 in .claude/settings.json
#   (https://code.claude.com/docs/en/agent-teams). This script runs one headless
#   session per invocation; it does not spawn product teammate sessions.
#
# Claude-specific env:
#   CLAUDE_PLAN_CLI           path to claude binary (default: claude on PATH)
#   CLAUDE_PLAN_ALLOWED_TOOLS comma-separated tools for -p (default: Bash,Read,Edit,Write).
#     Write is required for creating new files (e.g. artifacts); without it, -p mode can hang
#     waiting for permission no human can grant. Override with CLAUDE_PLAN_ALLOWED_TOOLS if needed.
#     Set CLAUDE_PLAN_NO_ALLOWED_TOOLS=1 to omit --allowedTools.
#     Optional per-agent `allowed_tools` in .claude/agents/<id>/config.json (string or JSON array)
#     is used when CLAUDE_PLAN_ALLOWED_TOOLS is unset. Env wins over agent config.
#
# Shared with Cursor ralph (CLAUDE_* or CURSOR_*; either prefix works):
...
#
# Logging:
#   Per-plan logs under .agents/logs/<artifact-namespace>/:
#   plan-runner-<planname>.log, plan-runner-<planname>-output.log
#   Set CLAUDE_PLAN_VERBOSE=1 or CURSOR_PLAN_VERBOSE=1 to print script events to stderr.
#   Override: CLAUDE_PLAN_LOG / CURSOR_PLAN_LOG, CLAUDE_PLAN_OUTPUT_LOG / CURSOR_PLAN_OUTPUT_LOG
#   Disable color: CLAUDE_PLAN_NO_COLOR=1 or CURSOR_PLAN_NO_COLOR=1
#
# Gutter: same TODO 10+ iterations; override *_PLAN_GUTTER_ITER (default 10).
# Progress: while Claude runs, first output line plus periodic "Agent still working (elapsed ...)"
#   every *_PLAN_PROGRESS_INTERVAL seconds (default 30).
...
if [[ "$(uname -s)" == "Darwin" ]] && \
   ...
   [[ "${CLAUDE_PLAN_NO_CAFFEINATE:-${CURSOR_PLAN_NO_CAFFEINATE:-0}}" != "1" ]] && \
   [[ "${CLAUDE_PLAN_CAFFEINATED:-${CURSOR_PLAN_CAFFEINATED:-0}}" != "1" ]]; then
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --agent)
      if [[ -z "${2:-}" ]]; then
        echo "Error: --agent requires a prebuilt agent name (subdirectory of .claude/agents/)." >&2
        exit 1
      fi
      PREBUILT_AGENT="$2"
      shift 2
...
# Shared config discovery: prefer runtime config, then `.ralph/plan-runner.json`.
if [[ -f "$WORKSPACE/.claude/ralph/plan-runner.json" ]]; then
...
MAX_ITERATIONS="${CLAUDE_PLAN_MAX_ITER:-${CURSOR_PLAN_MAX_ITER:-9999}}"
GUTTER_ITERATIONS="${CLAUDE_PLAN_GUTTER_ITER:-${CURSOR_PLAN_GUTTER_ITER:-10}}"
CLAUDE_PLAN_VERBOSE="${CLAUDE_PLAN_VERBOSE:-${CURSOR_PLAN_VERBOSE:-0}}"
CLAUDE_PLAN_NO_COLOR="${CLAUDE_PLAN_NO_COLOR:-${CURSOR_PLAN_NO_COLOR:-0}}"

if [[ -t 1 && "$CLAUDE_PLAN_NO_COLOR" != "1" ]]; then
  C_R="\033[31m"
  C_G="\033[32m"
  C_Y="\033[33m"
  C_B="\033[34m"
...
prompt_agent_source_mode() {
  ...
  echo "  2) Proceed with Claude default" >&2
...
_PLAN_LOG="${CLAUDE_PLAN_LOG:-${CURSOR_PLAN_LOG:-}}"
_PLAN_OUTPUT_LOG="${CLAUDE_PLAN_OUTPUT_LOG:-${CURSOR_PLAN_OUTPUT_LOG:-}}"
if [[ -z "$_PLAN_LOG" ]]; then
...
CLAUDE_CLI="${CLAUDE_PLAN_CLI:-}"
if [[ -z "$CLAUDE_CLI" ]] && command -v claude &>/dev/null; then
  CLAUDE_CLI="claude"
fi
if [[ -z "$CLAUDE_CLI" ]] || ! command -v "$CLAUDE_CLI" &>/dev/null; then
...
mkdir -p "$(dirname "$OUTPUT_LOG")"
{
  echo ""
  echo "################################################################################"
  echo "# Plan runner (Claude) started $(date '+%Y-%m-%d %H:%M:%S') | workspace=$WORKSPACE"
  echo "# Plan: $PLAN_PATH (log prefix: plan-runner-${PLAN_LOG_NAME})"
  echo "################################################################################"
} >> "$OUTPUT_LOG"

if [[ "$NON_INTERACTIVE_FLAG" == "1" && -z "$PREBUILT_AGENT" && -z "${CLAUDE_PLAN_MODEL:-${CURSOR_PLAN_MODEL:-}}" ]]; then
  log "ERROR: --non-interactive requires --agent <name> or CLAUDE_PLAN_MODEL"
  echo -e "${C_R}${C_BOLD}Non-interactive mode requires either a prebuilt agent or CLAUDE_PLAN_MODEL.${C_RST}" >&2
  exit 1
fi
...
if [[ ! -f "$PLAN_PATH" ]]; then
  ...
  echo -e "${C_DIM}Create it, set .claude/ralph/plan-runner.json (or .ralph/plan-runner.json), or pass --plan <path>.${C_RST}"
  exit 1
fi
...
if [[ -n "$PREBUILT_AGENT" ]]; then
  ...
  log "agent discovery (Claude): root=$_agents_root ids=[${_discovered:-none}]"
  if ! validate_prebuilt_agent_config "$WORKSPACE" "$PREBUILT_AGENT"; then
    echo -e "${C_R}Invalid agent config for '${PREBUILT_AGENT}'.${C_RST} See .claude/agents/README.md" >&2
  ...
  if [[ -n "${CLAUDE_PLAN_MODEL:-${CURSOR_PLAN_MODEL:-}}" ]]; then
    SELECTED_MODEL="${CLAUDE_PLAN_MODEL:-${CURSOR_PLAN_MODEL:-}}"
...
while [[ $iteration -lt $MAX_ITERATIONS ]]; do
  ...
  if [[ $ITERATIONS_ON_SAME_TODO -gt $GUTTER_ITERATIONS ]]; then
    ...
    echo ""
    echo -e "${C_DIM}Resolve or adjust the plan, then re-run this script. Override gutter limit with CLAUDE_PLAN_GUTTER_ITER or CURSOR_PLAN_GUTTER_ITER=N.${C_RST}"
    exit 1
  fi
...
When writing handoff artifacts, use the namespace-aware paths from the prebuilt agent context."
  ...
  log "invoking $CLAUDE_CLI (model=${SELECTED_MODEL:-default})${PREBUILT_AGENT:+ prebuilt_agent=$PREBUILT_AGENT}"
  ...
  PROGRESS_INTERVAL="${CLAUDE_PLAN_PROGRESS_INTERVAL:-${CURSOR_PLAN_PROGRESS_INTERVAL:-30}}"
  START_TIME="$(date +%s)"
  ...
  run_agent() {
    ...
    if [[ -n "$tools_use" ]]; then
      invoke+=(--allowedTools "$tools_use")
    fi
    printf '%s' "$PROMPT" | "$CLAUDE_CLI" "${invoke[@]}" 2>&1 | tee -a "$OUTPUT_LOG"
  ...
  set -e
  log "$CLAUDE_CLI finished (exit=$exit_code)"
