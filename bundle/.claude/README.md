...
When a plan runs for Claude Code you can opt in to CLI session resume so the next invocation picks up the stored assistant conversation. Enable it by setting `RALPH_PLAN_CLI_RESUME=1`, passing `--cli-resume`, or answering **yes** when the interactive prompt runs in a TTY. The runner records the active session ID at `.ralph-workspace/sessions/<RALPH_PLAN_KEY>/session-id.claude.txt` and the JSON demux helper (`.ralph/bash-lib/run-plan-cli-json-demux.py`) uses Python 3 to parse the `stream-json` output and update that file; without Python 3, the plan shrugs it off and continues without resume.
