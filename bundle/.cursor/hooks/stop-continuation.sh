#!/usr/bin/env bash
# Cursor stop hook adapter: wait for Ralph background jobs and emit followup_message.

set -uo pipefail

ralph_cursor_stop_hook_fail_open() {
  exit 0
}

ralph_cursor_stop_hook_workspace() {
  if [[ -n "${WORKSPACE:-}" ]]; then
    printf '%s\n' "$WORKSPACE"
    return 0
  fi
  jq -r '.workspace_roots[0] // .cwd // empty' <<<"${RALPH_CURSOR_STOP_HOOK_INPUT:-{}}"
}

ralph_cursor_stop_hook_main() {
  local workspace adapter_lib core_script hook_input

  command -v jq >/dev/null 2>&1 || ralph_cursor_stop_hook_fail_open

  hook_input="$(cat)" || hook_input="{}"
  RALPH_CURSOR_STOP_HOOK_INPUT="$hook_input"
  ...
  workspace="$(ralph_cursor_stop_hook_workspace)"
  [[ -n "$workspace" ]] || ralph_cursor_stop_hook_fail_open

  adapter_lib="$(ralph_native_hook_resolve_bash_lib "$workspace" "native-hook/stop-continuation-followup-adapter.sh" 2>/dev/null || true)"
  ...
  ralph_bg_stop_followup_adapter_main <<<"$hook_input"
}
