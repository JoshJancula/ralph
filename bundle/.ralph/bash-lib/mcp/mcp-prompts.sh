...
generate_next_todo_prompt_definition() {
  jq -n '{
    ...
    description: "Guides an orchestrator to pick the next unchecked TODO, select a runtime and optional instruction-only role, and describe the following plan invocation.",
    arguments: [
      {
        name: "workspace",
...
2. Run \`ralph role list --workspace $workspace\` when role guidance is useful; roles are instruction-only and do not select the runtime's agent.
3. Decide which runtime and optional role should execute the next unchecked TODO. Record the model source, nativeSubagents mode, and delegatedRunId when those values are supplied by the execution context.
4. Provide the upcoming \`ralph_run_plan\` call as JSON (workspace, plan_path, runtime, and optional role) so the orchestrator can follow your recommendation.
