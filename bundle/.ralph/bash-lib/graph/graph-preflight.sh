...
# Host probe/table helpers live in bash-lib/preflight so top-level commands
# (ralph doctor) can reuse them without sourcing graph-gate or report code.
if ! declare -F graph_preflight_format_table >/dev/null 2>&1; then
  # shellcheck source=../preflight/preflight-host.sh
  source "$GRAPH_PREFLIGHT_SCRIPT_DIR/../preflight/preflight-host.sh"
fi
