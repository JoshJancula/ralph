...
ralph_run_plan_record_workspace_registry() {
  ...
  helper="$lib_dir/../../python/workspace-registry.py"
  if [[ ! -f "$helper" ]]; then
...
ralph_run_plan_load_env_for_runtime() {
  ...
  case "$runtime" in
    ...
    opencode)
      ...
      no_open="${OPENCODE_PLAN_NO_OPEN:-${CLAUDE_PLAN_NO_OPEN:-${CURSOR_PLAN_NO_OPEN:-0}}}"
      ;;
    antigravity)
      verbose="${ANTIGRAVITY_PLAN_VERBOSE:-${OPENCODE_PLAN_VERBOSE:-${CODEX_PLAN_VERBOSE:-${CLAUDE_PLAN_VERBOSE:-${CURSOR_PLAN_VERBOSE:-0}}}}}"
      no_color="${ANTIGRAVITY_PLAN_NO_COLOR:-${OPENCODE_PLAN_NO_COLOR:-${CODEX_PLAN_NO_COLOR:-${CLAUDE_PLAN_NO_COLOR:-${CURSOR_PLAN_NO_COLOR:-0}}}}}"
      max_iter="${ANTIGRAVITY_PLAN_MAX_ITER:-${OPENCODE_PLAN_MAX_ITER:-${CODEX_PLAN_MAX_ITER:-${CLAUDE_PLAN_MAX_ITER:-${CURSOR_PLAN_MAX_ITER:-9999}}}}}"
      gutter="${ANTIGRAVITY_PLAN_GUTTER_ITER:-${OPENCODE_PLAN_GUTTER_ITER:-${CODEX_PLAN_GUTTER_ITER:-${CLAUDE_PLAN_GUTTER_ITER:-${CURSOR_PLAN_GUTTER_ITER:-10}}}}}"
      interval="${ANTIGRAVITY_PLAN_PROGRESS_INTERVAL:-${OPENCODE_PLAN_PROGRESS_INTERVAL:-${CODEX_PLAN_PROGRESS_INTERVAL:-${CLAUDE_PLAN_PROGRESS_INTERVAL:-${CURSOR_PLAN_PROGRESS_INTERVAL:-30}}}}}"
      plan_log="${ANTIGRAVITY_PLAN_LOG:-${OPENCODE_PLAN_LOG:-${CODEX_PLAN_LOG:-${CLAUDE_PLAN_LOG:-${CURSOR_PLAN_LOG:-}}}}}"
      plan_output_log="${ANTIGRAVITY_PLAN_OUTPUT_LOG:-${OPENCODE_PLAN_OUTPUT_LOG:-${CODEX_PLAN_OUTPUT_LOG:-${CLAUDE_PLAN_OUTPUT_LOG:-${CURSOR_PLAN_OUTPUT_LOG:-}}}}}"
      no_caffeinate="${ANTIGRAVITY_PLAN_NO_CAFFEINATE:-${OPENCODE_PLAN_NO_CAFFEINATE:-${CODEX_PLAN_NO_CAFFEINATE:-${CLAUDE_PLAN_NO_CAFFEINATE:-${CURSOR_PLAN_NO_CAFFEINATE:-0}}}}}"
      caffeinated="${ANTIGRAVITY_PLAN_CAFFEINATED:-${OPENCODE_PLAN_CAFFEINATED:-${CODEX_PLAN_CAFFEINATED:-${CLAUDE_PLAN_CAFFEINATED:-${CURSOR_PLAN_CAFFEINATED:-0}}}}}"
      disable_prompt="${ANTIGRAVITY_PLAN_DISABLE_HUMAN_PROMPT:-${OPENCODE_PLAN_DISABLE_HUMAN_PROMPT:-${CODEX_PLAN_DISABLE_HUMAN_PROMPT:-${CLAUDE_PLAN_DISABLE_HUMAN_PROMPT:-${CURSOR_PLAN_DISABLE_HUMAN_PROMPT:-0}}}}}"
      no_open="${ANTIGRAVITY_PLAN_NO_OPEN:-${OPENCODE_PLAN_NO_OPEN:-${CLAUDE_PLAN_NO_OPEN:-${CURSOR_PLAN_NO_OPEN:-0}}}}"
      ;;
    *)
