...
run_plan_invoke_codex_mcp_config_cleanup() {
  if [[ -n "${CODEX_PLAN_MCP_CONFIG_PATH:-}" ]]; then
...
run_plan_invoke_codex_mcp_config_prepare() {
  ...
  export CODEX_PLAN_MCP_CONFIG_PATH
...
ralph_run_plan_invoke_codex() {
  ...
  if [[ "$_ralph_mcp_active" == "1" ]]; then
    ...
    if [[ -n "$validation_error" ]]; then
      ...
      if grep -qE 'unknown configuration field.*mcp_servers\.ralph\.(enabled|command|args|env\.)' <<<"$validation_error"; then
        local unsupported_field
        unsupported_field="$(grep -oE 'mcp_servers\.ralph\.[a-zA-Z_\.]+' <<<"$validation_error" | head -1)"
