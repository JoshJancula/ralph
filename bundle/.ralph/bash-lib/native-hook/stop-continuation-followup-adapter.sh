#!/usr/bin/env bash
# Shared Cursor Stop hook serialization: neutral core -> followup_message.

ralph_bg_stop_followup_adapter_run_core() {
  local core_script="${1:-}"
  local hook_input="${2:-}"
...
ralph_bg_stop_followup_adapter_build_message() {
...
ralph_bg_stop_followup_adapter_emit_followup() {
  local neutral_json="${1:-}"
  local message
  message="$(ralph_bg_stop_followup_adapter_build_message "$neutral_json")"
  jq -nc --arg followup_message "$message" '{followup_message:$followup_message}'
}

ralph_bg_stop_followup_adapter_main() {
  ...
  neutral="$(ralph_bg_stop_followup_adapter_run_core "$core_script" "$hook_input")" || exit 0
  [[ -n "$neutral" ]] || exit 0

  decision="$(jq -r '.decision // empty' <<<"$neutral" 2>/dev/null || true)"
  if [[ "$decision" == "continue" ]]; then
    ralph_bg_stop_followup_adapter_emit_followup "$neutral"
  fi
  exit 0
}
