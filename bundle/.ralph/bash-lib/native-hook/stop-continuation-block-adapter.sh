#!/usr/bin/env bash
# Shared Claude/Codex Stop hook serialization: neutral core -> decision:block + reason.

ralph_bg_stop_block_adapter_run_core() {
  local core_script="${1:-}"
  local hook_input="${2:-}"
...
ralph_bg_stop_block_adapter_build_reason() {
...
ralph_bg_stop_block_adapter_emit_block() {
  local neutral_json="${1:-}"
  local reason
  reason="$(ralph_bg_stop_block_adapter_build_reason "$neutral_json")"
  jq -nc --arg reason "$reason" '{decision:"block",reason:$reason}'
}

ralph_bg_stop_block_adapter_main() {
  ...
  neutral="$(ralph_bg_stop_block_adapter_run_core "$core_script" "$hook_input")" || exit 0
  [[ -n "$neutral" ]] || exit 0

  decision="$(jq -r '.decision // empty' <<<"$neutral" 2>/dev/null || true)"
  if [[ "$decision" == "continue" ]]; then
    ralph_bg_stop_block_adapter_emit_block "$neutral"
  fi
  exit 0
}
