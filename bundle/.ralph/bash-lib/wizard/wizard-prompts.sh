...
pick_model_for_runtime() {
  ...
  case "$runtime" in
    ...
    opencode)
      ...
      ;;
    antigravity)
      if declare -F select_model_antigravity >/dev/null 2>&1; then
        select_model_antigravity --interactive
      fi
      ;;
  esac
}

_wizard_pick_required_model() {
  local runtime="$1"
  local agent="$2"
  local picked_model
  ...
  if [[ -z "$picked_model" ]]; then
    ralph_die "Model is required for $runtime stage with agent \"$agent\" (no agent default model configured)."
  fi
...
select_model_override() {
  ...
  # When the agent already declares a default model, use it without an extra prompt.
  # Only prompt for a model when there is no default to fall back on.
  if [[ -n "$model_default" ]]; then
    printf '%s' "$model_default"
    return
  fi

  _wizard_pick_required_model "$runtime" "$agent"
}
...
select_session_strategy() {
  ...
  ralph_menu_select --prompt "Session strategy for \"$stage\"" --default "$default_index" -- "fresh" "resume" "reset" "compact"
}

select_context_budget() {
  local stage="$1"
  ralph_menu_select --prompt "Context budget for \"$stage\"" --default 2 -- "full" "standard" "lean"
}
...
agent_model_default() {
  ...
  python3 "${SCRIPT_DIR}/python/wizard-prompts-agent-model.py" "$cfg"
}
