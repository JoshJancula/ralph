...
select_model_opencode() {
  ...
  if [[ "${1:-}" == "--batch" ]]; then
    shift
    _select_model_batch_dispatch opencode OPENCODE_PLAN_MODEL "$@"
    return
  fi
...
export -f select_model_opencode _opencode_select_model_interactive _opencode_list_models_from_cli 2>/dev/null || true
