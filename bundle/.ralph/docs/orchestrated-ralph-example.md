...
1. Copy `.ralph/plan-templates/classic.plan.template.md` into stage files:

   ```bash
   mkdir -p .ralph-workspace/orchestration-plans/feature
   cp .ralph/plan-templates/classic.plan.template.md .ralph-workspace/orchestration-plans/feature/feature-01-research.plan.md
   cp .ralph/plan-templates/classic.plan.template.md .ralph-workspace/orchestration-plans/feature/feature-02-architecture.plan.md
   cp .ralph/plan-templates/classic.plan.template.md .ralph-workspace/orchestration-plans/feature/feature-03-implementation.plan.md
   ```
...
```
I need a stage plan for [STAGE-TYPE] that lives in `.ralph-workspace/orchestration-plans/<namespace>/<namespace>-<NN>-<stage>.plan.md`. Base it on `.ralph/plan-templates/classic.plan.template.md`, break the work into discrete `- [ ]` TODOs, mention the files or modules touched, include validation commands (lint/test/build), and surface the artifact file under `.ralph-workspace/artifacts/<namespace>/` that you will deliver. The plan should be detailed enough that the orchestrator can run the stage and verify the artifacts automatically.
```

**Orchestration spec prompt**

```
I am coordinating [FEATURE] across multiple runtimes. Each stage plan already exists under `.ralph-workspace/orchestration-plans/<namespace>/`. Produce a pipeline plan at `.ralph-workspace/orchestration-plans/<namespace>/<namespace>.plan.md` with a `pipeline:` block. Wire the plans together in order, set a runtime and agent for each stage, point to the correct plan paths, list the required artifact files (research.md, architecture.md, implementation-handoff.md, etc.), and include `loopControl` when a reviewer should send work back to an earlier stage.
```

Set `runtime` per stage to decide if Cursor/Claude/Codex/OpenCode/Antigravity runs that piece. Antigravity model ids come from `agy models` and are passed unchanged to `agy --model "<exact model string from agy models>"`. Use `loopControl` to rerun implementation if the review stage signals `status: changes-required`.
...
2. Enter a pipeline name and namespace (for example `notifications`), then pick runtimes and agents for each stage. The wizard writes:
   ...
   - `.ralph-workspace/orchestration-plans/<namespace>/<namespace>.plan.md` (the pipeline plan)
...
4. If needed, edit the generated pipeline plan to add extra stages, enforce required artifacts, or configure `loopControl` for review/rework cycles.
5. Run the orchestrator using that generated pipeline plan and inspect `.ralph-workspace/logs/` plus `.ralph-workspace/artifacts/<namespace>/` after each stage.
...
```bash
ralph run --plan .ralph-workspace/orchestration-plans/feature/notifications-pipeline.plan.md
```
