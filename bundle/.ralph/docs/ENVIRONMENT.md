...
| `RALPH_CACHE_READ_PER_TURN_WARN` | `60000` | Threshold for the context-bloat hint emitted by `tool_call_target_telemetry.optimization_hint_line`. When the derived `cache_read_per_tool_turn` exceeds this integer cache-read token limit, Ralph emits the optimization hint that suggests trimming cached context or reducing tool turns. |
...
- When you rerun a declared verification manually, start it with `ralph_proxy_shell_start` and block on completion with `ralph_proxy_shell_wait` (pass `waitSeconds` to limit how long the server waits). Keeping the wait on the proxy server avoids repeated `ralph_proxy_shell_status` polls, which defeat the wait helper and flood the cache.
- Use `ralph_proxy_shell_start` plus `ralph_proxy_shell_wait` whenever you need to monitor a long exploratory shell job; leave `ralph_proxy_shell_status` as an occasional manual progress check, avoid short-interval polling loops, inspect output via `ralph_proxy_shell_read`, and cancel via `ralph_proxy_shell_cancel` if you must intervene. Treat `shell_wait` as the preferred blocker for follow-up work outside verification so `shell_status` can remain a human-initiated spot check rather than the default loop.
...
| `RALPH_OPENCODE_CACHE_DEBUG` | `0` (default). When `1` and the runtime is OpenCode, the JSON demux appends every token/usage-bearing event from the CLI stream to `opencode-usage-events.log` (beside the compact output log). Use it to tell a genuine cache miss from a cache hit the provider does not report, e.g. when ollama-cloud usage shows `cache_read=0`. |
| `RALPH_OPENCODE_USAGE_DEBUG_LOG` | Optional explicit file path for the OpenCode usage capture when `RALPH_OPENCODE_CACHE_DEBUG=1`. When unset, the log is written beside the compact output log. |
...
| `RALPH_PROXY_SHELL_ASYNC` | Defaults `1`. When Ralph MCP proxy tools are active, advertises `ralph_proxy_shell_start/wait/status/read/cancel` for long-running commands that exceed the host MCP request timeout. Runner-first verification remains the default path for commands proving TODO completion, so these async helpers exist only for exploratory or manual monitoring. Prefer `ralph_proxy_shell_wait` for blocking follow-up work, treat `ralph_proxy_shell_status` as an occasional manual status check, avoid short-interval polling loops, and inspect/cancel output through `ralph_proxy_shell_read` and `ralph_proxy_shell_cancel`. Set `0` to hide the async shell tools. |
...
| `RALPH_MCP_PROXY_BATCH_MAX_OPERATIONS` | Max operations per `ralph_proxy_batch` call (default `4`). |
| `RALPH_MCP_PROXY_BATCH_TIMEOUT_SEC` | Max seconds per `ralph_proxy_batch` call before timing out remaining operations (default `20`). Returns partial failure if timeout is exceeded. |
...
For long commands that cannot be declared as verification, use `ralph_proxy_shell_start` instead of a single blocking `ralph_proxy_shell` call. The runner-first policy still prefers metadata-driven verification, so only fall back to these async helpers when the job truly needs manual intervention. `ralph_proxy_shell_start` returns a `jobId` immediately; prefer `ralph_proxy_shell_wait` (optionally with `waitSeconds`) so you wait for completion without hammering the proxy. Treat `ralph_proxy_shell_status` as a manual follow-up for occasional progress checks, avoid short-interval polling loops, inspect output with `ralph_proxy_shell_read`, and cancel with `ralph_proxy_shell_cancel` if needed. The same shell policy and `proxyOwnedTools.shellTimeoutSeconds` limit apply.
...
| **MCP fallback (recommended)** | OpenCode | Plugin staging and code verified (T6/T7), but headless hook invocation and output mutation unproven (OpenCode 1.14.35). `hybrid` keeps MCP compaction authoritative and records native hook effectiveness as unproven until that changes. | `--ralph-mode hybrid` or `ralph` with `RALPH_PROXY_SHELL_COMPACT=1` |
...
| **OpenCode** (1.14.35) | UNPROVEN headless invocation | UNPROVEN headless invocation | MCP fallback (recommended) | Plugin code implements both hooks; staging proven; headless firing and output mutation not proven |
...
**OpenCode (documented local plugin path; native adapters unproven headless):** Verified on OpenCode **1.14.35** (2026-06-04): Ralph stages `bundle/.opencode/plugins/ralph-runtime-hooks.ts` to workspace `.opencode/plugins/` (documented auto-discovery path proven via T6). Plugin code implements `tool.execute.before` and `tool.execute.after` (T7 verified). However, headless `opencode run` hook invocation and agent-visible output mutation are unproven (upstream issues indicate hooks may not fire on headless agent invocation). Overlay records `native_hooks_configured=true`, `native_hooks_effective=false`, `native_hooks_reason=plugin_injected_unproven_headless`, `fallback_path_active=true`. **Use MCP proxy compaction (`RALPH_PROXY_SHELL_COMPACT=1`, `--ralph-mode ralph`, `ralph_proxy_shell`) as the authoritative path for OpenCode.** See `bundle/.opencode/plugins/SPIKE-output-mutation.md`.
