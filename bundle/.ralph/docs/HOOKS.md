...
| `bundle/.cursor/hooks/post-tool-shell-telemetry.sh` | Cursor | `postToolUse` | `Shell` | `RALPH_BASH_TELEMETRY_LOG` (unset by default; observability only) | Appends a JSONL audit line (byte counts, command hash) to the configured telemetry log; does **not** attempt output replacement because Cursor's `updated_tool_output` is proven not to change agent-visible Shell stdout. | Exits 0 always; missing `jq` or unset `RALPH_BASH_TELEMETRY_LOG` simply skips logging. |
| `bundle/.cursor/hooks/post-tool-native-result-compact.sh` | Cursor | `postToolUse` | `Read\|read\|readToolCall\|Grep\|grep\|grepToolCall\|Glob\|glob\|globToolCall\|SemanticSearch\|semanticSearch` | `RALPH_NATIVE_RESULT_COMPACT` (default off; explicit opt-in) | Delegates to the same shared `post-tool-native-result-compact-hook.sh` used by Claude/Codex to compact native exploration output and replace it via `updated_tool_output`. | Exits 0 (leaves output untouched) when the shared hook is missing, the gate is off, or on any internal failure. |
| `bundle/.cursor/hooks/post-tool-mcp-compact.sh` | Cursor | `postToolUse` | `MCP:*` (scoped internally to `ralph_proxy_*`) | `RALPH_CURSOR_MCP_HOOK_COMPACT` (explicit override) falling back to `RALPH_PROXY_SHELL_COMPACT` (default off) | Compacts Ralph MCP proxy tool results that exceed the per-tool byte cap and replaces them via `updated_mcp_tool_output`, storing the original for later retrieval; records windowing telemetry when the shared telemetry lib is present. **Only `ralph_proxy_*` tools are ever touched; third-party MCP results pass through unchanged.** | Exits 0 (leaves the MCP result untouched) when disabled, the tool isn't a `ralph_proxy_*` tool, the result is already an envelope, the result is under the byte cap, or any library fails to load. |
| `bundle/.cursor/hooks/after-shell-telemetry.sh` | Cursor | `afterShellExecution` | n/a (fires for every shell execution) | duration learning always (no env gate); `RALPH_BASH_TELEMETRY_LOG` (unset by default) for optional audit lines | Records shell duration from the payload's documented `duration` field into the shared `command_profiles` store; optionally appends a one-line audit record when the telemetry log path is set. | Exits 0 always; missing `jq`/workspace/python skips learning silently; unset `RALPH_BASH_TELEMETRY_LOG` skips only the audit line. |
| `bundle/.cursor/hooks/stop-continuation.sh` | Cursor | `stop` | n/a | none directly; underlying core respects background-job state | Same background-job wait/block behavior as the Claude Stop hook, adapted for Cursor's `stop` hook shape (`loop_limit: 5` in `hooks.json`). | Exits 0 (lets the run stop normally) on missing dependencies or unresolved project directory. |
| `bundle/.agents/hooks/pre-tool-shell-policy.sh` | Antigravity (agy, `.agents/hooks.json` `ralph-native`) | `PreToolUse` | `run_command` | `RALPH_NATIVE_SHELL_WRAPPER` (wrapper) or `RALPH_BASH_REWRITE` (plain rewrite), both gates independent of Cursor | Reads agy's camelCase stdin (`toolCall.args.CommandLine`, `workspacePaths[0]`), evaluates the killswitch, and answers `{"decision":"deny","reason":...}` or `{"decision":"allow"}`; when a rewrite applies it adds `overwrite: {CommandLine: ...}`. Rewrite telemetry is recorded on this PreToolUse side. | Always prints `{"decision":"allow"}` and exits 0 on missing `jq`, unresolved workspace, missing libraries, gates off, or no rule match. |
| (none registered) | Antigravity | `PostToolUse` | n/a | n/a | agy's PostToolUse payload carries only `stepIdx` and `error`, so Ralph registers no PostToolUse hook; a hook answering here could only return `{}`. | n/a |
| `bundle/.agents/hooks/stop-continuation.sh` | Antigravity | `Stop` | n/a | none directly; underlying core respects background-job state | Reads `workspacePaths[0]` and `fullyIdle` from agy's stdin and emits `{"decision":"continue","reason":...}` only while Ralph background jobs are running; otherwise emits `{}`. Uses `stop-continuation-continue-adapter.sh`, which no other runtime shares. | Prints `{}` and exits 0 on missing dependencies, unresolved workspace, or `fullyIdle: false`. |

Antigravity has no output-replacement hook and no shell-duration hook. agy does not deliver tool output or elapsed time to any hook, so result compaction (native Read/Grep/Glob and MCP proxy compaction, shell output replacement) and shell duration learning are unavailable on agy. The Antigravity rows above are the complete agy hook set; none of the Cursor scripts above run under agy.
...
| Antigravity | `run_command` | None | Antigravity's native `run_command` tool does not expose a background mode. Use the Ralph fallbacks. |
...
| Antigravity | **No** — agy hooks receive no elapsed-time field (PostToolUse carries only `stepIdx` and `error`; there is no shell-duration hook) | None; duration learning is unavailable on agy |
