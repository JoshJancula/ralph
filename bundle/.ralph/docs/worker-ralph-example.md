...
1. Copy the template:
   ...
   ```bash
   cp .ralph/plan-templates/classic.plan.template.md PLAN.md
   ```
...
2. Supply overrides as needed:
   ...
   - For Claude/Codex without env overrides, add saved defaults: `ralph models add claude <id>` (store: `${RALPH_CONFIG_HOME:-${XDG_CONFIG_HOME:-$HOME/.config}/ralph}/models.json`).
