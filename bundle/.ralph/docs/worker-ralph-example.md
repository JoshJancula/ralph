...
3. If the runner pauses for human input: `.ralph/run-plan.sh` (always pass **`--plan`**) follows an **interactive-first flow** where TTY-attached runs prompt you inline. Without a TTY, it polls `operator-response.txt` in-process (set `RALPH_HUMAN_OFFLINE_EXIT=1` to use exit 4 and restart instead). Optional escalation via `.ralph/orchestrator.sh --human-ack` or `RALPH_HUMAN_ACK_TOOL` still applies when configured. Human exchanges are persisted under **`.ralph-workspace/sessions/<RALPH_PLAN_KEY>/human-replies.md`** (and related files in that directory) for audit and replay purposes.
