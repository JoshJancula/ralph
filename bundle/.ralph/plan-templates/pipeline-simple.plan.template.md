...
todos:
  - id: example-task
    ...
    verification: Run the verification steps that prove this task is complete (build, test, lint, or equivalents). Keep fixing failures until they pass, and report the result with `TODO_VERIFICATION: PASS` or `TODO_VERIFICATION: FAIL: <reason>` as appropriate.
    status: pending
  - id: example-task-2
