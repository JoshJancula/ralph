...
# Reuse the single guarded verification-command extractor so post-verification
# only ever runs allow-listed commands and never raw natural-language prose.
...
def command_post_verify(todo_text: str) -> int:
    # Only emit an explicit strict Verify: command. Natural-language
    # Verification: prose yields no command; the runner then asks the agent to
    # run the verification steps and report a result instead of executing prose.
    command = verify_to_complete_command(todo_text)
    if command:
        print(command)
        return 0
    return 1
...
def plan_verify_command(path: str) -> int:
    ...
    for line in frontmatter.splitlines():
        match = re.match(r"^\s*verify\s*:\s*(.+\S)\s*$", line, re.I)
        if match:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    post_verify = sub.add_parser("post-verify-command")
    post_verify.add_argument("--todo", required=True)
    plan_verify = sub.add_parser("plan-verify-command")
    ...
    if args.cmd == "post-verify-command":
        return command_post_verify(args.todo)
    if args.cmd == "plan-verify-command":
        return plan_verify_command(args.plan)
    return 1
