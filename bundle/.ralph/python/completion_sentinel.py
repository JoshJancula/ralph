#!/usr/bin/env python3
"""Detect assistant completion markers in assistant-authored text only."""
...
TODO_COMPLETE_MARKER = "TODO_COMPLETION"
_BULLET_PREFIX_RE = re.compile(r"^[\*●\-]\s+")
_MARKER_BOUNDARY_RE = re.compile(
    r"^" + re.escape(AGENT_DONE_MARKER) + r"(?:\s|$|[^\w])"
)
_TODO_COMPLETE_RE = re.compile(
    r"^" + re.escape(TODO_COMPLETE_MARKER) + r"\s*:\s*COMPLETE(?:\s|$)",
    re.IGNORECASE,
)
...
def line_has_completion_sentinel(line: str) -> bool:
    ...
    if stripped == AGENT_DONE_MARKER:
        return True
    if _TODO_COMPLETE_RE.match(stripped):
        return True
