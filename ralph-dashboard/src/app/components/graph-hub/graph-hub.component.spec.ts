    ...
    nodes: [
      {
        ...
        dependsOn: [],
        derivedFrom: 'stage',
        stage: { id: 'source', runtime: 'claude', _inlineTodos: [] },
      },
      {
        id: 'analyze',
        type: 'stage',
...
describe('GraphHubComponent', () => {
  ...
  it('nests native helpers and brokered children while showing deduplicated run usage', fakeAsync(() => {
    const fixture = TestBed.createComponent(GraphHubComponent);
    const component = fixture.componentInstance;
    fixture.detectChanges();
    ...
    observed.usage = {
      ...
      brokeredChildren: { inputTokens: 4 },
      total: { input_tokens: 10, inputTokens: 4 },
    };
    ...
    observed.nodes[0].nativeSubagentMode = 'read-only';
    observed.nodes[0].brokeredChildren = [{ delegationId: 'delegation-source-1', runtime: 'codex', status: 'awaiting-ack' }];
    observed.nodes[0].nativeSubagentEvents = [{ event: 'native-subagent-finished', timestamp: '2026-01-01T00:00:09Z' }];
    ...
    const compiled: HTMLElement = fixture.nativeElement as HTMLElement;
    expect(compiled.textContent).toContain('brokered children:');
    expect(compiled.textContent).toContain('native helpers:');
    ...
    expect(compiled.querySelector('.dag-pre')?.textContent).toContain('children (ledger/runtime-owned)');
  }));
