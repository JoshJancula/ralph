/**
 * Read-through cache for the workflow read endpoints.
 *
...
const cache = new Map<string, { readonly fingerprint: string; readonly value: unknown }>();
...
/**
 * Returns the cached value for `key` when `fingerprint` still matches,
 * otherwise runs `producer` and stores its result. A throwing producer is not
 * cached, so transient CLI failures do not become sticky.
 */
