...
@test "run-plan Ralph Mode guidance explains stored-result view choices" {
  [ -f "$RUN_PLAN_SH" ] || skip "bundle run-plan missing"
  [ -x "$(command -v python3)" ] || skip "python3 required"

  run python3 - "$REPO_ROOT/bundle/.ralph/bash-lib/run-plan/run-plan-core.sh" <<'PY'
import sys
...
required = [
    ...
    "build/test/lint output, package-manager installs",
    "package-manager installs",
    "before raw reads when you only need errors",
    "source files, generated code, structured data",
    "Prefer compacted/search follow-ups for logs and command output",
]
missing = [needle for needle in required if needle not in text]
if missing:
    print("missing guidance: " + ", ".join(missing))
    raise SystemExit(1)
PY
...
EOF
  ...
  local xdg_cache_dir
  xdg_cache_dir="$(mktemp -d)"

  run bash -c '
    set -euo pipefail
    PATH="$1"
    export PATH
    export XDG_CACHE_HOME="$5"
  ...
  ' _ "$stub_dir:$PATH" "$capture_file" "$RUN_PLAN_SH" "$plan_file" "$xdg_cache_dir"
  ...
  [[ "$env_output" == *"/usr/bin/env"* ]] || [[ "$env_output" == *"ralph-run-plan"* ]]
...
@test "ralph path to file uri uses sample paths" {
  ...
  run bash -c '
    ...
    eval "$(sed -n "/^ralph_path_to_file_uri()/,/^}/p" "$1")"
    printf "%s" "$(ralph_path_to_file_uri "$2")"
  ' _ "$REPO_ROOT/bundle/.ralph/bash-lib/run-plan/run-plan-core.sh" "$path_space"
