...
run_overlay_cleanup_case() {
  ...
  local workspace plan_file stub_dir marker registry_file
...
EOF
  ...
  run env -u RALPH_AGENT_TOOL_ACCESS -u RALPH_LAUNCHER_PID \
    ...
    bash "$RUN_PLAN_SH" --runtime cursor --plan "$plan_file" --workspace "$workspace" --agent research --non-interactive
