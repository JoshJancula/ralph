...
@test "prompt_agent_source_mode '2' selects direct model entry, not prebuilt agent" {
  [ -f "$RUN_PLAN_SH" ] || skip "bundle run-plan missing"
...
EOF
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"agent_flag=0 model_flag=1"* ]]
  rm -f "$prompt_funcs" "$runner"
}

@test "prompt_agent_source_mode '1' selects a prebuilt agent" {
  [ -f "$RUN_PLAN_SH" ] || skip "bundle run-plan missing"
...
EOF
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"agent_flag=1 model_flag=0"* ]]
  rm -f "$prompt_funcs" "$runner"
}
