...
@test "cleanup-plan wrapper removes namespace data and rejects missing namespace" {
  ...
  tool_results_dir="$workspace/.ralph-workspace/tool-results/$namespace"

  mkdir -p "$log_dir" "$legacy_log_dir" "$session_dir" "$legacy_session_dir" "$artifact_dir" "$tool_results_dir/results"
  touch "$log_dir/plan-runner-1" "$legacy_log_dir/plan-runner-old" "$artifact_dir/file" "$tool_results_dir/results/0123456789abcdef.txt"
  ...
  [[ "$output" == *"Removed tool results"* ]]
  [[ "$output" == *".ralph-workspace/tool-results/$namespace"* ]]
