...
HELPER="$REPO_ROOT/bundle/.ralph/python/plan-split.py"
...
@test "plan frontmatter verify command is exposed" {
  command -v python3 >/dev/null || skip "python3 required"
  cat >"$PLAN_FILE" <<'EOF'
---
verify: echo post-verify
---
# Plan
- [ ] Do the thing
EOF

  run python3 "$HELPER" plan-verify-command --plan "$PLAN_FILE"

  [ "$status" -eq 0 ]
  [ "$output" = "echo post-verify" ]
}
