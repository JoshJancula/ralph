...
1. a **task** (`--task "..."`), or
2. a **file** (`--plan <path>`) — any plain-text or Markdown file.
...
ralph workflow start feature-delivery --plan ./specs/csv-export.md
```

`--plan` accepts any plain-text or Markdown file. Ralph inspects the file for
Ralph-style TODOs (`- [ ]` items):

- **No TODOs found:** the file is treated as a free-form task description. Its
  content becomes the task text for the workflow. No special formatting required
  — requirements docs, design notes, AI-generated specs, and plain prose all work.
- **TODOs found:** the file is treated as a Ralph leaf plan and used as immutable
  plan input (requires `planInput` on the workflow definition).
...
A workflow that pins no runtime is resolved at start: attended starts prompt for
a runtime and model, non-interactive starts require `--runtime`. See
[Runtime and model precedence](#runtime-and-model-precedence).
...
Seven workflows are directly runnable (project, global, or bundled resolution).
Choose by intent:
...
Consensus voters keep explicit runtimes; supervisors take none.

### Resolving a routing-neutral workflow at start

Bundled workflows pin no routing: no `defaults:` block and no stage `runtime:`.
`ralph workflow start` resolves that before it creates anything.

Attended starts walk through:

1. **Select a runtime** for the unresolved stages (installed runtimes only).
2. **Select a model**, or accept the runtime default. Claude offers your saved
   models followed by `haiku`, `sonnet`, `opus`, and `fable` -- aliases the
   Claude Code CLI resolves to the latest model in each family. Accepting the
   runtime default writes the stage with a runtime only, and the runtime picks
   its own model when the stage runs.
3. **Apply to every stage, or choose per stage.** Per stage prompts for a
   runtime and model for each remaining unresolved stage.
4. **Optionally save the selection into the workflow source.** Applying to every
   stage writes a `defaults:` block; a per-stage selection writes `runtime:` and
   `model:` onto each stage. The updated source is validated before it replaces
   the original, and only project-scoped sources are written -- run
   `ralph workflow edit <id> --project` first to get a writable copy of a
   bundled or global workflow.

The resolved routing is printed, then the usual start confirmation runs. A
declined save still applies the selection to that run.

Non-interactive starts (`--yes`, or stdin that is not a terminal) do not prompt.
They fail before the run registry entry exists, naming the unresolved stages and
pointing at `--runtime`/`--model` or a `defaults:` block.
...
run `resume` after the reset. This is a manual additional repair/review cycle,
not stale-supervisor recovery, and it can be repeated if another final review
still requests changes.

For any failed run, `status` identifies the best recorded code-producing stage,
its workspace mode and absolute path, whether that path still exists, whether it
is a Git worktree, its frozen base revision, its durable changeset manifest, and
the changed-file count. `ralph workflow handoff <run-id>` expands those facts
into a standalone report with the exact retry, resume, verdict, log, and
open-workspace commands. Another Ralph process can use the same run from the
same project and state root. Runtime/model routing remains frozen; changing it
requires a new run. Handoff does not export a run to another machine or state
root.
