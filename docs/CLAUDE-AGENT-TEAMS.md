...
1. You (or the lead) have a Ralph plan (e.g. `PLAN.md` or a stage plan under `.agents/orchestration-plans/`).
2. Ask Claude to create an agent team and break the plan into parallel tasks. For example:
   ...
   - One teammate focuses on the research TODOs and writes findings to .agents/artifacts/{{ARTIFACT_NS}}/research.md
   - One teammate focuses on implementation TODOs and runs the verification commands (lint/test) from the plan
   - One teammate does security review and documents issues in .agents/artifacts/{{ARTIFACT_NS}}/security-review.md
   ```
...
Have them each write findings to .agents/artifacts/pr-142/review-<focus>.md and then discuss to produce a single .agents/artifacts/pr-142/synthesis.md.
```

Give each teammate enough context in the spawn prompt: paths to relevant files, artifact locations (e.g. `.agents/artifacts/{{ARTIFACT_NS}}/...`), and any Ralph conventions (e.g. no emoji, repo-context skill). Teammates load project context (e.g. CLAUDE.md, MCP, skills) but not the lead’s conversation history, so include task-specific details in the prompt.
...
- **Ralph artifact paths**: Use `.agents/artifacts/{{ARTIFACT_NS}}/...` (or a fixed namespace) so outputs are in the same place the orchestrator and run-plan expect. Replace `{{ARTIFACT_NS}}` with your pipeline or plan namespace (e.g. `notifications`, `pr-142`).
...
- **Artifact handoffs**: Have each subagent write to Ralph-style artifact paths (e.g. `.agents/artifacts/<namespace>/`) so results are discoverable and can feed into later stages.
...
Task(description="Security review", prompt="Review PR #142 for token handling, session management, input validation. Write findings to .agents/artifacts/pr-142/review-security.md")
Task(description="Performance analysis", prompt="Profile PR #142 for performance impact. Write findings to .agents/artifacts/pr-142/review-performance.md")
Task(description="Test coverage analysis", prompt="Check test coverage for PR #142. Write findings to .agents/artifacts/pr-142/review-coverage.md")
...
- Write artifacts following Ralph conventions (`.agents/artifacts/...`).
...
2. **Export team results to Ralph artifacts**: Teammates write findings to `.agents/artifacts/<namespace>/...`.
